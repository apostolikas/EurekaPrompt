...
def create_model_input(task, question, instruction, choices, narrative):
    ...
    elif task in ['csqa', 'social_iqa', 'causal_judg']:
        input_prompt = f'''Question: {question}\nAnswer Choices: {choices}\nAnswer: {instruction}'''
        model_input = f'''GPT4 Correct User: {input_prompt}<|end_of_turn|>GPT4 Correct Assistant:'''

    elif task == 'date_under':
        input_prompt = f'''Infer the date from the context.\nQuestion: {question}\nAnswer choices: {choices}\nAnswer: {instruction}\n<COMPLETE>'''
        model_input = f'''GPT4 Correct User: {model_input}<|end_of_turn|>GPT4 Correct Assistant:'''

    elif task == 'sports_und':
        input_prompt = f'''Determine whether an artificially constructed sentence relating to sports is plausible or not.\nQuestion: {question}\nAnswer choices: {choices}\nAnswer: {instruction}\n<COMPLETE>'''
        model_input = f'''GPT4 Correct User: {input_prompt}<|end_of_turn|>GPT4 Correct Assistant:'''

    elif task == 'disamb':
        input_prompt = f'''Clarify the meaning of sentences with ambiguous pronouns.\nQuestion: {narrative}\nAnswer choices: {choices}\nAnswer: {instruction}\n'''
        model_input = f'''GPT4 Correct User: {input_prompt}<|end_of_turn|>GPT4 Correct Assistant:'''    
...
def generate_response(task, decode_strategy, model, tokenizer, question, instruction, choices=None, narrative=None):
    ...
    if decode_strategy == 'greedy':
        outputs = model.generate(**inputs, do_sample = False, num_beams = 1, max_new_tokens = 800, pad_token_id = tokenizer.pad_token_id, eos_token_id = tokenizer.eos_token_id)
        generated_text= tokenizer.batch_decode(outputs, skip_special_tokens=True)

    elif decode_strategy == 'contrastive_search':
        outputs = model.generate(**inputs, penalty_alpha=0.6, top_k=4, max_new_tokens = 800, pad_token_id = tokenizer.pad_token_id, eos_token_id = tokenizer.eos_token_id)
        generated_text= tokenizer.batch_decode(outputs, skip_special_tokens=True)

    elif decode_strategy == 'multinomial_sampling':
        outputs = model.generate(**inputs, do_sample=True, num_beams=1, max_new_tokens = 800, pad_token_id = tokenizer.pad_token_id, eos_token_id = tokenizer.eos_token_id)
        generated_text= tokenizer.batch_decode(outputs, skip_special_tokens=True)

    elif decode_strategy == 'beam_search':
        outputs = model.generate(**inputs, num_beams=5, do_sample = False, max_new_tokens = 800, pad_token_id = tokenizer.pad_token_id, eos_token_id = tokenizer.eos_token_id)
        generated_text= tokenizer.batch_decode(outputs, skip_special_tokens=True)

    elif decode_strategy == 'beam_search_with_multinomial_sampling':
        outputs = model.generate(**inputs, num_beams=5, do_sample=True, max_new_tokens = 800, pad_token_id = tokenizer.pad_token_id, eos_token_id = tokenizer.eos_token_id)
        generated_text= tokenizer.batch_decode(outputs, skip_special_tokens=True)

    elif decode_strategy == 'top_k_sampling':
        outputs = model.generate(**inputs, do_sample=True, top_k=50, max_new_tokens = 800, pad_token_id = tokenizer.pad_token_id, eos_token_id = tokenizer.eos_token_id)
        generated_text= tokenizer.batch_decode(outputs, skip_special_tokens=True)
    
    elif decode_strategy == 'top_p_sampling':
        outputs = model.generate(**inputs, do_sample=True, top_p=0.9, max_new_tokens = 800, pad_token_id = tokenizer.pad_token_id, eos_token_id = tokenizer.eos_token_id)
        generated_text= tokenizer.batch_decode(outputs, skip_special_tokens=True)

    elif decode_strategy == 'sampling0.25':
        outputs = model.generate(**inputs, do_sample=True, top_k = 0, max_new_tokens = 800, pad_token_id = tokenizer.pad_token_id, eos_token_id = tokenizer.eos_token_id, temperature=0.25)
        generated_text= tokenizer.batch_decode(outputs, skip_special_tokens=True)
    
    elif decode_strategy == 'sampling0.5':
        outputs = model.generate(**inputs, do_sample=True, top_k = 0, max_new_tokens = 800, pad_token_id = tokenizer.pad_token_id, eos_token_id = tokenizer.eos_token_id, temperature=0.5)
        generated_text= tokenizer.batch_decode(outputs, skip_special_tokens=True)

    elif decode_strategy == 'sampling0.75':
        outputs = model.generate(**inputs, do_sample=True, top_k = 0, max_new_tokens = 800, pad_token_id = tokenizer.pad_token_id, eos_token_id = tokenizer.eos_token_id, temperature=0.75)
        generated_text= tokenizer.batch_decode(outputs, skip_special_tokens=True)
...
if __name__ == '__main__':
    ...
    model = AutoModelForCausalLM.from_pretrained("berkeley-nest/Starling-LM-7B-alpha", device_map='auto', torch_dtype=torch.bfloat16)
            ...
            for prompt in prompts:
                ...
                for decode_strategy in decode_strategies:
                    for i, sample in tqdm(enumerate(samples)):
                        ...
                        elif task == 'abs_nar' or task == 'disamb':
