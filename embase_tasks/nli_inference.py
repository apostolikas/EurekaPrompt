...
def construct_icl_examples(samples, number_of_examples, seed):
    ...
    for icl_sample in icl_samples:
        premise = icl_sample['premise']
        hypothesis = icl_sample['hypothesis']
...
def inference(args, model, tokenizer, nli_prompt):

    random.seed(args.seed)
    testset = EmbaseNLI().data
    testset = random.sample(testset,100)
    ...
    filename = f"./Starling_Embase_NLI_{args.mode}_{args.ins}_seed_{args.seed}.txt"

    with open(filename, 'w') as f:

        f.write(f"MODE:{args.mode}\tINSTRUCTION:{args.ins}\n")
        f.write(f"NLI instruction: {nli_prompt}")
        ...
        for i,sample in enumerate(testset):
            ...
            if args.ins == "icl":
                ...
                if args.mode == '0shot':
                    model_input = f"Q: Choose entailment, contradiction or neutral for the relationship between the premise and the hypothesis.\n\nPremise: {premise}\n\nHypothesis: {hypothesis}\n\nA:"

                elif args.mode == '1shot':
                    icl_examples = construct_icl_examples(testset, 1, args.seed)
                    model_input = f"{icl_examples}Q: Choose entailment, contradiction or neutral for the relationship between the premise and the hypothesis.\n\nPremise: {premise}\n\nHypothesis: {hypothesis}\n\nA:"
                ...
                if args.mode == '0shot':
                    ...
                    ins_examples = construct_ins_examples(testset, 1, args.seed, nli_prompt)
                    model_input = f"{ins_examples}Q: Choose entailment, contradiction or neutral for the relationship between the premise and the hypothesis.\n\nPremise: {premise}\n\nHypothesis: {hypothesis}\n\n{nli_prompt}\n\nA:"

                elif args.mode == '3shot':
                    ins_examples = construct_ins_examples(testset, 3, args.seed, nli_prompt)
                    model_input = f"{ins_examples}Q: Choose entailment, contradiction or neutral for the relationship between the premise and the hypothesis.\n\nPremise: {premise}\n\nHypothesis: {hypothesis}\n\n{nli_prompt}\n\nA:"

                elif args.mode == '5shot':
                    ins_examples = construct_ins_examples(testset, 5, args.seed, nli_prompt)
                    model_input = f"{ins_examples}Q: Choose entailment, contradiction or neutral for the relationship between the premise and the hypothesis.\n\nPremise: {premise}\n\nHypothesis: {hypothesis}\n\n{nli_prompt}\n\nA:"
            ...
            result = evaluate_nli(text_output, label)
            acc += result
...
if __name__ == "__main__":
    ...
    for mod in modes:
        for ins in instructions:
            args.mode = mod
            args.ins = ins
            acc = inference(args, model, tokenizer, nli_prompt=None)
