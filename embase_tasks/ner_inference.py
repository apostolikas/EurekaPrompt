...
def inference(args, model, tokenizer, ner_prompt):
    ...
    filename = f"./Starling_Embase_NER_{args.mode}_{args.ins}_seed_{args.seed}.txt"
    ...
    with open(filename, 'w') as f:
        ...
        for i,sample in enumerate(testset):
            ...
            text = sample['input']
            label = sample['structured_output']
            entity_type = sample['entity_type']
            
            if args.ins == "icl":

                if args.mode == '0shot':
                    model_input = f"Q: What {entity_type}s can be found in the text?\n\nText: {text}\n\nA:"

                elif args.mode == '1shot':
                    icl_examples = construct_icl_examples(testset, 1, args.seed)
                    model_input = f"{icl_examples}Q: What {entity_type}s can be found in the text?\n\nText: {text}\n\nA:"
                ...
                if args.mode == '0shot':
                    ...
                    model_input = f"{ins_examples}Q: What {entity_type}s can be found in the text?\n\nText: {text}\n\n{ner_prompt}\n\nA:"
