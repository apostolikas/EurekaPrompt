...
def evaluate_oqa(y_pred:str, label:str):
    pass
...
def construct_icl_examples(samples, number_of_examples, seed):
    ...
    for icl_sample in icl_samples:
        question = icl_samples['question']
        context = icl_sample['context']
...
def inference(args, model, tokenizer, oqa_prompt):
    ...
    with open(filename, 'w') as f:
        ...
        f.write(f"OQA instruction: {oqa_prompt}")
        ...
        for i,sample in enumerate(testset):
            ...
            if args.ins == "icl":
                ...
                if args.mode == '0shot':
                    ...
                    ins_examples = construct_ins_examples(testset, 1, args.seed, oqa_prompt)
                    model_input = f"{ins_examples}{instruction}\n\nQuestion: {question}\n\nContext: {context}\n\n{oqa_prompt}\n\nAnswer: {label}\n\n"

                elif args.mode == '3shot':
                    ins_examples = construct_ins_examples(testset, 3, args.seed, oqa_prompt)
                    model_input = f"{ins_examples}{instruction}\n\nQuestion: {question}\n\nContext: {context}\n\n{oqa_prompt}\n\nAnswer: {label}\n\n"

                elif args.mode == '5shot':
                    ins_examples = construct_ins_examples(testset, 5, args.seed, oqa_prompt)
                    model_input = f"{ins_examples}{instruction}\n\nQuestion: {question}\n\nContext: {context}\n\n{oqa_prompt}\n\nAnswer: {label}\n\n"
            ...
            result = evaluate_oqa(text_output, label)
            acc += result
...
if __name__ == "__main__":
    ...
    for mod in modes:
        for ins in instructions:
            args.mode = mod
            args.ins = ins
            acc = inference(args, model, tokenizer, oqa_promp=None)
