...
def evaluate_cqa(y_pred:str, label:str):
    pass
...
def inference(args, model, tokenizer, cqa_prompt):

    random.seed(args.seed)
    testset = ReaxysCQA().data
    ...
    filename = f"./Starling_Reaxys_CQA_{args.mode}_{args.ins}_seed_{args.seed}.txt"

    with open(filename, 'w') as f:

        f.write(f"MODE:{args.mode}\tINSTRUCTION:{args.ins}\n")
        f.write(f"CQA instruction: {cqa_prompt}")
        ...
        for i,sample in enumerate(testset):
            ...
            if args.ins == "icl":
                ...
                if args.mode == '0shot':
                    ...
                    ins_examples = construct_ins_examples(testset, 1, args.seed, cqa_prompt)
                    model_input = f"{ins_examples}Question: {question}\n\n{cqa_prompt}\n\nAnswer: {answer}\n\n"

                elif args.mode == '3shot':
                    ins_examples = construct_ins_examples(testset, 3, args.seed, cqa_prompt)
                    model_input = f"{ins_examples}Question: {question}\n\n{cqa_prompt}\n\nAnswer: {answer}\n\n"

                elif args.mode == '5shot':
                    ins_examples = construct_ins_examples(testset, 5, args.seed, cqa_prompt)
                    model_input = f"{ins_examples}Question: {question}\n\n{cqa_prompt}\n\nAnswer: {answer}\n\n"
            ...
            result = evaluate_cqa(text_output, answer)
            acc += result
