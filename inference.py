...
if __name__ == "__main__":
    ...
    initial_population = load_inference_prompts(args.task)
    ...
    for prompt in initial_population:
        start = time.time()
        fitness_dict[prompt], prompts_results[prompt] = inference_engine.evaluate_prompt(prompt)
        end = time.time()
    ...
    with open(f"./inference_logs/{args.task}_{args.model}_answers.txt", 'w') as f:
        for prompt in prompts_results:
            f.write(f"Prompt: {prompt}\n")
            for i, result in enumerate(prompts_results[prompt]):
